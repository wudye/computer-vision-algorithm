...
def main():
    ...
    candidates = [base_dir / "circle.png", base_dir / "circle.jpg"]
    image_path = next((p for p in candidates if p.exists()), None)
    if image_path is None:
        raise FileNotFoundError(
            f"No input image found. Tried: {[str(p) for p in candidates]}"
        )

    src = cv.imread(str(image_path), 0)
    if src is None:
        raise FileNotFoundError(f"Failed to decode image file: {image_path}")
